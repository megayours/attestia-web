...
interface AuthContextType {
  ...
  login: () => Promise<void>;
  logout: () => void;
  createOrg: (ipIdentifier: string) => Promise<void>;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function createOrg(ipIdentifier: string) {
    const { user: updated } = await createOrgApi(ipIdentifier);
    setUser(updated);
  }

  return (
    <AuthContext.Provider value={{ user, loading, register, login, logout, createOrg }}>
      {children}
